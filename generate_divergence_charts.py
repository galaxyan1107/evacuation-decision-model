...
# Output directory
OUTPUT_DIR = Path("/sessions/youthful-nifty-lamport/mnt/flea/gitbook-evacuation-model/charts")
OUTPUT_DIR.mkdir(exist_ok=True)

def load_data():
    """Load daily_data.json and run_log.json"""
    daily_path = Path("/sessions/youthful-nifty-lamport/mnt/flea/gitbook-evacuation-model/pipeline/daily_data.json")
    run_log_path = Path("/sessions/youthful-nifty-lamport/mnt/flea/gitbook-evacuation-model/pipeline/run_log.json")
...
def generate_lambda_evolution(run_log):
    ...
    ax.set_title(f'Lambda Evolution - Queue Stability Across {max(days)} Days',
                fontsize=CHART_CONFIG['title_size'], fontweight='bold', pad=20)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / f'lambda_evolution_day{max(days)}.png', dpi=CHART_CONFIG['dpi'], bbox_inches='tight')
    plt.close()
    print(f"Generated: lambda_evolution_day{max(days)}.png")
...
def main():
    ...
    chart_files = [
        ...
        OUTPUT_DIR / 'lambda_evolution_day53.png',
