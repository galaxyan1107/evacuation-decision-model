...
| # | 检查项 | 模型 | 第20天观测 | 第21天观测 | 状态 |
|---|--------|------|-----------|-----------|------|
| 1 | 弹道导弹单调递减 | 是 | 7枚 | **4枚** | **⚠️ 偏离**（降至4枚，平第16天最低；仍远超模型~0） |
| 2 | 拦截率>90%（累计） | 93.2% | ~92.8%（日100%） | **~92.9%（日100%）** | **⚠️ 稳定**（连续第4天100%日拦截率） |
| 3 | 无人机率~130/天 | ~130/天 | 15/天 | **26/天** | **⚠️ 极端偏离**（−80%；历史低位小幅回升） |
| 4 | 无新武器类型 | 无 | 卡塔尔LNG（区域） | **无新类型** | **偏离**（今日无新武器；前期升级仍存） |
| 5 | 停火概率（Polymarket） | 84% | ~10% | **~8%** | **偏离**（历史新低；开斋节无停火） |
| 6 | 机场恢复 | 55%（第12天） | ~45% | **~40%** | **⚠️ 偏离**（模型74% vs 实际40%；1.85倍差距；外国航司禁飞） |
| 7 | 无人机库存>30% | ~20% | ~14.3% | **~13.0%** | **⚠️ 严重**（~260架剩余；接近耗尽） |
| 8 | 霍尔木兹海峡开放 | P=98%开放 | 关闭（~12次通行） | **关闭（~15次通行）** | **偏离**（选择性通行扩大但仍有效关闭） |
| 9 | 无代理人激活 | P=96%未激活 | 卡塔尔LNG被毁；冲突扩大 | **美国考虑释放伊朗原油** | **偏离**（区域冲突持续） |
| 10 | 判定 | 亚稳态 | 不稳定（λ=2.161） | **不稳定（λ=2.163）** | **偏离**（λ持平；锁定稳定级联） |
