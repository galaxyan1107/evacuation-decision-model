...
逐日6项指标百分比偏差。红色=实际超出模型，蓝色=实际低于模型。Lambda偏离从第3天起主导（+240% → +402%）。

![偏离热力图](../../charts/divergence_heatmap.png)

### 6面板对比

模型（蓝色）vs实际（红色）并排对比。机场（绿色）是唯一正向偏离。Lambda（右下）显示级联区突破。

![模型vs实际](../../charts/model_vs_actual_bars.png)

### 记分卡与判定时间线

堆叠偏离显示Lambda（紫色）主导总模型误差。判定时间线：模型全8天预测亚稳态——现实在第3天跨入不稳定且未恢复。

![偏离记分卡](../../charts/divergence_scorecard.png)

### Lambda演变

λ在第3天从0.47跳至1.70（霍尔木兹关闭），第8天达2.49（空军基地被袭+弹道反弹）。P(λ>1)从第3天起一直为100%。

![Lambda演变](../../charts/lambda_evolution_day8.png)

### 弹道导弹轨迹

模型的指数衰减假设（β=0.25/天）从第5天起失效。第5→8天：3→7→9→16呈加速反弹，推翻TEL消耗假说。
...
|------|-------|-------|-------|-------|-------|------|
| 发射装置消耗 | ~39% | ~50% | ~54% | 85.7% | ~73%↓ | > 85% |
| 无人机库存 | 89.6% | 65.6% | 53.0% | 40.8% | 34.5% | < 30% |
...
| 8 | 3月7日 | 导弹 | **16枚弹道导弹**（第2天以来最高） | **λ_弹道反弹+0.30，消耗修正至73%** |
...
|---|--------|------|----------|----------|------|
| 1 | 弹道导弹单调递减 | 是 | 7→9（打破） | 9→16（⚠️加速） | **严重偏离** |
...
尽管机场恢复提供了正向信号，系统级稳定性已**急剧恶化**。弹道导弹反弹（第5→8天：3→7→9→16）推翻了导弹衰减假设。霍尔木兹关闭、代理人激活、空军基地被袭加上弹道导弹加速已将冲突推入级联领域。改善的机场运力提供了一个**狭窄的撤离窗口**——应立即利用。
